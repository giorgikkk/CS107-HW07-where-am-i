...
(define (partition pivot seq cmp)
  (if (null? seq) '(() ())
      (let ((split-of-rest (partition pivot (cdr seq) cmp)))
	(if (cmp (car seq) pivot)
	    (list (cons (car seq) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car seq) (car (cdr split-of-rest))))))))
...
(define (quicksort seq cmp)
  (if (<= (length seq) 1) seq
      (let ((split (partition (car seq) (cdr seq) cmp)))
...
(define (intersection-points circle-list)
  (if (or (null? circle-list) (equal? '() (cdr circle-list))) '()
    (append (append (intersect (car circle-list) (cadr circle-list))
                    (intersection-points (cons (car circle-list) (cddr circle-list))))
            (intersection-points (cdr circle-list)))))


(define (distance-product point point-list)
  (apply * (map (lambda (p) (dist point p)) 
                (remove point point-list))))


(define (rate-points point-list)
  (map cons (map (lambda (p) (distance-product p point-list)) 
                  point-list) 
            (map (lambda (p) (list p)) point-list)))


(define (cmp first-list sec-list)
  (< (car first-list) (car sec-list)))


(define (sort-points rate-point-list)
  (quicksort rate-point-list cmp))


(define (clumped-points point-list)
  (prefix-of-list (map (lambda (x) (car x)) 
                       (map (lambda (p) (cdr p)) 
                            (sort-points (rate-points point-list)))) 
                  (if (= (remainder (length point-list) 2) 1) (- (/ (length point-list) 2) 0.5)
                    (/ (length point-list) 2))))


(define (avg-point point-list)
  (cons (/ (apply + (map (lambda (p) (car p)) point-list)) (length point-list)) 
        (list (/ (apply + (map (lambda (p) (cadr p)) point-list)) (length point-list)))))


(define (average-point point-list)
  (let ((avg (avg-point point-list)))
    (cons (distance-product avg point-list) 
          (list avg))))


(define (best-estimate point-list)
  (average-point (clumped-points (intersection-points point-list))))


(define (where-am-i distance-list star-location-list)
  (sort-points (map (lambda (x) (best-estimate x)) 
                    (all-guesses distance-list star-location-list))))
